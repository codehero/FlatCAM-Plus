...
class ToolMilling(Excellon, AppTool):
    ...
    def on_tool_add(self, dia=None, new_geo=None):
        ...
        for db_tool, db_tool_val in tools_db_dict.items():
            ...
            if db_tool_val['data']['tool_target'] not in [1, _('Milling')]:
                continue
    ...
    def ensure_milling_tool_defaults(self, geo_obj, tools_dict):
        milling_defaults = {}

        for option, value in self.app.options.items():
            if option.startswith('tools_mill_'):
                milling_defaults[option] = deepcopy(value)

        milling_defaults['seg_x'] = deepcopy(self.app.options.get('geometry_seg_x', 0))
        milling_defaults['seg_y'] = deepcopy(self.app.options.get('geometry_seg_y', 0))
        milling_defaults['geometry_seg_x'] = deepcopy(self.app.options.get('geometry_seg_x', 0))
        milling_defaults['geometry_seg_y'] = deepcopy(self.app.options.get('geometry_seg_y', 0))

        obj_options = getattr(geo_obj, 'obj_options', {})
        for option, value in obj_options.items():
            if option.startswith('tools_mill_') or option in ['seg_x', 'seg_y', 'geometry_seg_x', 'geometry_seg_y']:
                milling_defaults[option] = deepcopy(value)

        if 'tools_mill_min_power' not in milling_defaults:
            milling_defaults['tools_mill_min_power'] = 0.0
        if 'tools_mill_laser_on' not in milling_defaults:
            milling_defaults['tools_mill_laser_on'] = 'M3'

        for option, value in milling_defaults.items():
            obj_options.setdefault(option, deepcopy(value))

        for tool_uid, tool_data in tools_dict.items():
            ...
            missing_required = [
                opt for opt in [
                    ...
                    'tools_mill_travelz',
                    'tools_mill_feedrate',
                    'tools_mill_feedrate_z',
                    'tools_mill_feedrate_rapid',
                    'tools_mill_min_power',
                    'tools_mill_laser_on',
                    'tools_mill_spindlespeed',
                    'tools_mill_spindledir',
                    'tools_mill_dwell',
                    'tools_mill_dwelltime',
                    'tools_mill_startz',
                    'tools_mill_endz',
                    'tools_mill_endxy',
                    'tools_mill_toolchange',
                    'tools_mill_toolchangez',
                    'tools_mill_toolchangexy',
                    'tools_mill_extracut',
                    'tools_mill_extracut_length',
                    'tools_mill_ppname_g',
                    'tools_mill_optimization_type',
                    'tools_mill_search_time'
                ] if opt not in data
            ]
            if missing_required:
                self.app.log.warning(
                    "ToolMilling.ensure_milling_tool_defaults() --> Tool %s still misses: %s" %
                    (tool_uid, ', '.join(missing_required))
                )
    ...
    def generate_cnc_job_handler(self, geo_obj=None, outname=None, tools_dict=None, tools_in_use=None,
        ...
        self.ensure_milling_tool_defaults(geo_obj, tools_dict)
        ...
        def resolve_milling_offset(tool_data, dia_value):
            ...
            if offset_key in ('custom', '3', 3):
                ...
                if offset_value in (None, '') and not from_tcl:
                    try:
                ...
                try:
                    ...
                    try:
                        offset_value = float(str(offset_value).replace(',', '.'))
                    except ValueError:
                        self.app.inform.emit('[ERROR_NOTCL] %s' %
