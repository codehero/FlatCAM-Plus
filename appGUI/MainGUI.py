...
from appVersion import APP_NAME

from appGUI.GUIElements import *
from appGUI.PanelStyles import (
    apply_modern_main_workspace_style, apply_modern_preferences_style, apply_modern_sidebar_style
)
...
class MainGUI(QtWidgets.QMainWindow):
    ...
    def configure_toolbar(self, toolbar, compact=False):
        ...
        if compact:
            toolbar.setStyleSheet(self.compact_toolbar_style_sheet())
    ...
    def configure_popup_menu(self, menu):
        menu.setToolTipsVisible(True)
        menu.setStyleSheet(self.toolbar_menu_style_sheet())
        ...
        menu.setAttribute(QtCore.Qt.WidgetAttribute.WA_NoSystemBackground, True)
    ...
    def add_toolbar_dropdown(self, toolbar, icon_name, tooltip, actions, text=None):
        ...
        button.setIcon(QtGui.QIcon(self.app.resource_location + icon_name))
        button.setText(text if text else tooltip)
        ...
        button.setProperty("popupButton", True)
        button.setMinimumWidth(toolbar.iconSize().width() + 18)
    ...
    def add_options_toolbar_controls(self):
        ...
        self.toolbar_rotate_dropdown, self.toolbar_rotate_dropdown_action, self.toolbar_rotate_menu = \
            self.add_toolbar_dropdown(
                self.toolbarplugins, '/rotate.png', _("Rotate"), [self.menuoptions_transform_rotate])

        self.toolbar_skew_dropdown, self.toolbar_skew_dropdown_action, self.toolbar_skew_menu = \
            self.add_toolbar_dropdown(
                self.toolbarplugins, '/skewX.png', _("Skew"),
                [self.menuoptions_transform_skewx, self.menuoptions_transform_skewy])

        self.toolbar_flip_dropdown, self.toolbar_flip_dropdown_action, self.toolbar_flip_menu = \
            self.add_toolbar_dropdown(
                self.toolbarplugins, '/flipx.png', _("Flip"),
                [self.menuoptions_transform_flipx, self.menuoptions_transform_flipy])

        self.toolbar_zoom_dropdown, self.toolbar_zoom_dropdown_action, self.toolbar_zoom_menu = \
            self.add_toolbar_dropdown(
                self.toolbarplugins, '/zoom_fit32.png', _("Zoom"),
                [self.zoom_fit_btn, self.zoom_in_btn, self.zoom_out_btn])

        self.toolbar_grid_dropdown, self.toolbar_grid_dropdown_action, self.toolbar_grid_menu = \
            self.add_toolbar_dropdown(
                self.toolbarplugins, '/grid32.png', _("Grid"),
                [
    ...
    def add_cnc_toolbar_controls(self):
        current_action = getattr(self, "cnc_toolbar_dropdown_action", None)
        if current_action is not None and current_action in self.toolbarplugins.actions():
            return

        self.cnc_toolbar_btn = self.toolbar_action(
            '/cnc32.png', _("CNC Control"), _("Open CNC Controller."))
    ...
    def add_ai_toolbar_controls(self):
        current_action = getattr(self, "ai_dropdown_action", None)
        if current_action is not None and current_action in self.toolbarplugins.actions():
            return
    ...
    def __init__(self, app):
        ...
        self.menuedit_move2origin.setToolTip(
            _("Move selected object(s) so the lower-left corner is at X0 Y0.")
        )
        ...
        self.configure_popup_menu(self.menuproject)
        ...
        self.file_open_excellon_btn = self.toolbar_action(
            '/open_excellon32.png', _("Open Excellon"), _("Open a Excellon file."))
        self.file_open_dropdown, self.file_open_dropdown_action, self.file_open_dropdown_menu = \
            self.add_toolbar_dropdown(
                self.toolbarfile, '/folder32.png', _("Open"),
                [self.file_open_btn, None, self.file_open_gerber_btn, self.file_open_excellon_btn])
        ...
        self.copy_btn = self.toolbar_action('/copy_file32.png', _("Copy"), _("Copy a selection of objects."))
        self.delete_btn = self.toolbar_action('/trash32.png', _("Delete"), _("Delete a selection of objects."))
        self.edit_selection_dropdown, self.edit_selection_dropdown_action, self.edit_selection_menu = \
            self.add_toolbar_dropdown(
                self.toolbaredit, '/copy_file32.png', _("Selection"), [self.copy_btn, self.delete_btn])

        self.toolbaredit.addSeparator()
        self.distance_btn = self.toolbar_action(
            '/distance32.png', _("Distance"), _("Measure a distance between two or more points."))
        ...
        self.origin_btn = self.toolbar_action('/origin32.png', _('Set Origin'), _("Set an origin to a custom location."))
        ...
        self.jmp_btn = self.toolbar_action(
            '/jump_to32.png', _('Jump to'), _("Move the mouse cursor to a defined position and center into view."))
        self.locate_btn = self.toolbar_action(
            '/locate32.png', _('Locate'), _("Locate a predefined position on a selected object."))
        self.edit_measure_dropdown, self.edit_measure_dropdown_action, self.edit_measure_menu = \
            self.add_toolbar_dropdown(
                self.toolbaredit, '/distance32.png', _("Measure and Locate"),
                [self.distance_btn, self.origin_btn, self.jmp_btn, self.locate_btn])

        # ########################################################################
        # ########################## View Toolbar# ###############################
        # ########################################################################
        self.replot_btn = self.toolbar_action('/replot32.png', _("Replot"))
        self.clear_plot_btn = self.toolbar_action('/clear_plot32.png', _("Clear Plot"))
        ...
        # ########################## Shell Toolbar# ##############################
        # ########################################################################
        self.shell_btn = self.toolbar_action('/shell32.png', _("Command Line"))
        self.new_script_btn = self.toolbar_action('/script_new24.png', '%s ...' % _('New Script'))
        self.open_script_btn = self.toolbar_action('/open_script32.png', '%s ...' % _('Open Script'))
        self.run_script_btn = self.toolbar_action('/script16.png', '%s ...' % _('Run Script'))
        self.script_dropdown, self.script_dropdown_action, self.script_menu = \
            self.add_toolbar_dropdown(
                self.toolbarshell, '/shell32.png', _("Shell and Scripts"),
                [self.shell_btn, None, self.new_script_btn, self.open_script_btn, self.run_script_btn])
        ...
        self.align_dropdown, self.align_dropdown_action, self.align_menu = \
            self.add_toolbar_dropdown(self.toolbarplugins, '/align32.png', _("Align"), [self.align_btn])
        ...
        apply_modern_sidebar_style(self.plugin_scroll_area, self.app)
        ...
        apply_modern_main_workspace_style(self, self.app)
        ...
        self.popmenu_move2origin.setStatusTip(
            _("Move selected object(s) to canvas origin using the lower-left corner as reference.")
        )
        self.popmenu_move = self.popMenu.addAction(
            QtGui.QIcon(self.app.resource_location + '/move32.png'), _("Move"))
        self.popmenu_properties = self.popMenu.addAction(
            QtGui.QIcon(self.app.resource_location + '/properties32.png'), _("Properties"))
        self.configure_popup_menu(self.popMenu)
        ...
        apply_modern_preferences_style(self, self.app)
    ...
    def set_ui_title(self, name):
        ...
        """
        title = '%s %s %s - %s - [%s]    %s' % (
            APP_NAME, self.app.version, ('BETA' if self.app.beta else ''),
            platform.architecture()[0], self.app.engine, name)
