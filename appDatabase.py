...
class ToolsDB2(QtWidgets.QWidget):
    ...
    TOOL_TARGET_PREFIXES = {
        0: ("tools_",),
        1: ("tools_mill_",),
        2: ("tools_drill_",),
        3: ("tools_iso_", "tools_mill_"),
        4: ("tools_paint_", "tools_mill_"),
        5: ("tools_ncc_", "tools_mill_"),
        6: ("tools_cutout_", "tools_mill_")
    }
    ...
    def tool_target_id(self, target):
        ...
        if isinstance(target, str):
            for target_id, target_name in self.TOOL_TARGET_NAMES.items():
                if target in [target_name, _(target_name), str(target_id)]:
                    return target_id
        return 0

    def tool_target_label(self, target):
        return _(self.TOOL_TARGET_NAMES.get(self.tool_target_id(target), "General"))

    def normalize_tool_preset(self, tool_record):
        """
        Tools DB is the preset source for tool-specific machining data.
        Preferences are only used here as default/fallback values when a preset
        is new or when an older DB entry misses keys.
        """

        if not isinstance(tool_record, dict):
            return tool_record

        data = tool_record.setdefault('data', {})
        if not isinstance(data, dict):
            data = {}
            tool_record['data'] = data

        data.setdefault("plot", True)
        data.setdefault("tool_target", 0)
        data.setdefault("tol_min", 0.0)
        data.setdefault("tol_max", 0.0)
        ...
        data.setdefault("seg_x", deepcopy(self.app.options.get("geometry_seg_x", 0)))
        ...
        data.setdefault("tools_drill_min_power", deepcopy(self.app.options.get("tools_drill_min_power", 0.0)))
        data.setdefault("tools_drill_laser_on", deepcopy(self.app.options.get("tools_drill_laser_on", "M3")))
    ...
    def on_list_selection_change(self, current, previous):
        ...
        self.normalize_tool_preset(self.db_tool_dict[current.text(0)])
    ...
    def storage_to_form(self, dict_storage):
        self.ui_disconnect()
        self.normalize_tool_preset(dict_storage)
    ...
    def setup_db_ui(self):
        ...
        for tool in self.db_tool_dict.values():
            self.normalize_tool_preset(tool)
    ...
    def build_db_ui(self):
        ...
        for toolid, dict_val in self.db_tool_dict.items():
            ...
            self.normalize_tool_preset(dict_val)
            op_name = self.tool_target_label(dict_val['data']['tool_target'])
    ...
    def on_tool_target_changed(self, index=None, val=None):
        ...
        if val is None:
            ...
            tool_target = val
        tool_target = self.tool_target_id(tool_target)

        if self.current_toolid is not None and str(self.current_toolid) in self.db_tool_dict:
            tool_record = self.db_tool_dict[str(self.current_toolid)]
            tool_record.setdefault('data', {})['tool_target'] = tool_target
            self.normalize_tool_preset(tool_record)
    ...
    def on_tool_add(self):
        ...
        self.normalize_tool_preset(dict_elem)
        ...
        for idx in range(self.app_ui.plot_tab_area.count()):
            if self.app_ui.plot_tab_area.tabText(idx) == _("Tools Database"):
                ...
                for tool_id in self.db_tool_dict.keys():
                    ...
                    tool_target = self.tool_target_id(self.db_tool_dict[tool_id]['data']['tool_target'])
                    ...
                    if tool_target == 1:
                        for k in list(self.db_tool_dict[tool_id]['data'].keys()):
                            if str(k).startswith('tools_'):
                                if str(k).startswith('tools_mill'):
                                    pass
                                else:
    ...
    def update_tree_target(self):
        ...
        op_name = self.tool_target_label(val)
    ...
    def update_storage(self):
        ...
        if wdg_name == "gdb_name":
            ...
            if wdg_name == "gdb_tool_target":
                ...
                self.db_tool_dict[tool_id]['data']['tools_mill_offset_value'] = val
        ...
        for item in self.ui.tree_widget.selectedItems():
            ...
            for key in self.db_tool_dict.keys():
                if str(key) == str(tool_uid):
                    selected_tool = deepcopy(self.normalize_tool_preset(self.db_tool_dict[key]))
